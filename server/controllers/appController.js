...
export async function register(req, res) {
    ...
    try {
        const { username, password, email, firstName, lastName, mobile, profile } = req.body;
        ...
        const existMobile = new Promise((resolve, reject) => {
            UserModel.findOne({ mobile }, function (err, mobile) {
                if (err) reject(new Error(err))
                if (mobile) reject({ error: "Please use unique mobile" });
        ...
        Promise.all([existUsername, existEmail, existMobile])
            .then(() => {
                if (password) {
